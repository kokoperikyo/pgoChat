<template>
  <div>
    ...
    <v-dialog v-model="deleteImgDialog" class="mx-auto" max-width="320">
      <v-card class="pa-3">
        <div>チャットをご利用中にチャット内容が表示されなくなることがあります。</div>
        <div>その際はこちらのボタンを押してください</div>
      </v-card>
    </v-dialog>
    <!-- パーティー名の編集 -->
      ...
      <v-card class="mx-auto px-2 pb-3" :color="leagueBaseColor">
        ...
        <v-row align="center">
          ...
          <v-col cols="2" align="center">
            <v-btn dark icon large @click="partyDialog = true,partyDialogSt = true">
    ...
    <!-- パーティー選択申請を受けたがわ -->
    <v-dialog v-model="selectSixThreeDialogOfOther" max-width="340" persistent>
      <v-card class="mx-auto px-2 pb-3" :color="leagueBaseColor">
        <v-row class="ml-0">
          <h3 class="font-weight-black mt-2" style="color:white;">６匹選んでください</h3>
          <v-col>
    ...
    <v-card flat tile id="chatCard">
      <v-toolbar height="40px" color="#8AC32B">
        ...
        <v-spacer></v-spacer>
        <v-btn class="mr-2" @click="partyDialog = true , partyDialogSt = false" icon small dark>
          <v-icon>mdi-view-list</v-icon>
        </v-btn>
        ...
        <v-btn class="ml-1" icon small dark @click="deleteImg">
...
export default {
  ...
  methods: {
    ...
    selectParty(st) {
      if (st == 1) {
        this.partyList = this.partyList1;
      } else if (st == 2) {
        this.partyList = this.partyList2;
      } else if (st == 3) {
        this.partyList = this.partyList3;
      } else if (st == 4) {
        this.partyList = this.partyList4;
      } else if (st == 5) {
        this.partyList = this.partyList5;
      }
  ...
  mounted() {
    ...
    setTimeout(() => {
      this.partyList1 = this.partyListDb[0].partyList;
      this.partyList2 = this.partyListDb[1].partyList;
      ...
      this.partyName4 = this.partyListDb[3].partyName;
      this.partyName5 = this.partyListDb[4].partyName;
    }, 2000);
