  ...
  <v-app>
    ...
    <v-app-bar clipped-left app absolute color="#DE4097" flat dark height="40px">
      ...
      <v-toolbar-items v-if="userStatus">
        ...
        <child v-on:friendRequestNotion="getFriendRequestNotion"></child>
      </v-toolbar-items>
...
import Child from "@/components/pages/Tes.vue";
...
export default {
  ...
  methods: {
    getFriendRequestNotion() {
      ...
      console.log("getFriendRequestNotion");
      this.neww = true;
      // db.collection("users")
  ...
  firestore() {
    // return {
    //   test: db.collection("users").doc(this.$store.getters.user.uid)
    // };
  }
};
</script>
